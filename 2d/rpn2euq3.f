...
c     #### not yet working on curvilinear grid!
c
...
c
        do i=2-mbc,mx+mbc
           alf(i) = auxl(i,inx)
           beta(i) = auxl(i,iny)
           q2l(i) = alf(i)*ql(i,2) + beta(i)*ql(i,3)
           q2r(i-1) = alf(i)*qr(i-1,2) + beta(i)*qr(i-1,3)
           q3l(i) = -beta(i)*ql(i,2) + alf(i)*ql(i,3)
           q3r(i-1) = -beta(i)*qr(i-1,2) + alf(i)*qr(i-1,3)
           enddo
c
c
...
c           # rotate momentum components of waves back to x-y:
	    wave2 = alf(i)*wave(i,2,mw) - beta(i)*wave(i,3,mw)
	    wave3 = beta(i)*wave(i,2,mw) + alf(i)*wave(i,3,mw)
	    wave(i,2,mw) = wave2
	    wave(i,3,mw) = wave3
   80       continue
c	 
...
c        # flux difference must also be rotated and scaled:
	 amdq2 = (alf(i)*amdq(i,2) - beta(i)*amdq(i,3)) 
	 amdq3 = (beta(i)*amdq(i,2) + alf(i)*amdq(i,3))
c
