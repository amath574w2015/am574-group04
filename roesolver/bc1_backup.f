...
!     Define variables in 1st interior cell
      rho1 = q(1,1)
      u1 = q(2,1)/q(1,1)
      p1 = gamma_m*(q(3,1)-0.5d0*u1**2.d0*rho1)/area(1)     
      c1 = sqrt(gamma*p1*area(1)/rho1)
...
!         if u is positive, two right going waves
     ...
     &          *c1*(1.d0-p0/p1)/sqrt(1.d0+beta*p0/p1)
             ...
             e0 = p0/gamma_m*area(1)+0.5d0*u0**2.d0*rho0
...
!          velocity positive, one left going wave
     ...
     &          /sqrt(1.d0+beta*patm/pN)
             ...
             eN1 = patm/gamma_m*area(mx)+0.5d0*uN1**2.d0*rhoN1
...
!          velocity negative, two left going waves
     ...
     &          /sqrt(1.d0+beta*patm/pN)
             ...
             q(1,mx+ibc) = rhoN1*area(mx)
             q(2,mx+ibc) = rhoN1*uN1*area(mx)
             q(3,mx+ibc) = eN1*area(mx)
           endif
